...
#         att_ws = att_w if att_ws is None else tf.concat([att_ws, att_w], 1)
    ...
    att_temp3 = tf.matmul(output2, att_w2)
    ...
    temp = tf.nn.softmax_cross_entropy_with_logits(logits=output, labels=tf.one_hot(labels[:,i], len(encodeWords)))
    temp *= masks[:,i]
    temp = tf.reduce_sum(temp) / 50.0
    loss += temp
...
for j in range(200):
    ...
    if j%1==0 and j!=0:
        ...
        saver.save(sess, "model/model"+str(j)+".ckpt")